...
exec(const char *prog, const char **argv)
{
	...
	for (i = 0; i < elf->e_phnum; i++, ph++) {
		...
		if (ph->p_flags & ELF_PROG_FLAG_WRITE)
			perm |= PTE_W;
		if ((r = map_segment(0, now_map + PGOFF(ph->p_va), ph->p_memsz,
				     fd, ph->p_filesz, ph->p_offset, perm)) < 0)
			goto error;
		now_map += ROUNDUP(PGOFF(ph->p_va) + ph->p_memsz, PGSIZE);
	}
	close(fd);
	fd = -1;

	if ((r = init_stack(0, argv, &tf_esp, now_map)) < 0)
		return r;

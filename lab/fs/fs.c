...
file_block_walk(struct File *f, uint32_t filebno, uint32_t **ppdiskbno, bool alloc)
{
       ...
       if(filebno < NDIRECT){
            // One of direct block pointers
            *ppdiskbno = &(f->f_direct[filebno]);
       }
       else{
            // Not a direct block pointer
            if(f->f_indirect){
                uint32_t *indir = diskaddr(f->f_indirect);
                *ppdiskbno = &indir[filebno-NDIRECT];
            }
            else{
                ...
                uint32_t *indir = diskaddr(blockno);
                *ppdiskbno = &(indir[filebno - NDIRECT]);
            }
       }
       return 0;

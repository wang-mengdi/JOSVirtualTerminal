...
trap_init(void)
{
    ...
    for(int i=0;i<=T_SYSCALL;i++){
        int dpl=0;
        if(i==T_BRKPT||i==T_SYSCALL) dpl=3;
        SETGATE(idt[i],0,GD_KT,vectors[i],dpl);
    }
	trap_init_percpu();
}
...
trap_dispatch(struct Trapframe *tf)
{
    ...
    switch(tf->tf_trapno){
        ...
        case T_SYSCALL: //system call
            ret=
            syscall(
                tf->tf_regs.reg_eax,//system call number
                tf->tf_regs.reg_edx,
                tf->tf_regs.reg_ecx,
                tf->tf_regs.reg_ebx,
                tf->tf_regs.reg_edi,
            ...
            if(ret<0){
                panic("system call invalid in trap_dispatch: %e\n",ret);
            }
...
page_fault_handler(struct Trapframe *tf)
{
	...
	// LAB 3: Your code here.
    if((tf->tf_cs&3)==0){
        // See 80386 Chapter 5.1, the lowest 2 bits of a selector is privilege level
        // If RPL==0, then it's in kernel mode.
